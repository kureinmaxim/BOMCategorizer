...
def extract_tu_code(text: str) -> Tuple[str, str]:
    ...
    # ===================================================================
    # НАШИ КОДЫ (без "ТУ"): ГВАТ/АМФИ/ИГНД/деX...
    # Нужно разделять "Название + код" -> (Название, код) для колонки "ТУ/Производитель".
    # Примеры:
    # - "Плата П8 АМФИ.687281.080" -> ("Плата П8", "АМФИ.687281.080")
    # - "Вентиль ГВАТ.434823.015"  -> ("Вентиль", "ГВАТ.434823.015")
    # - "Зажим де4.835.001"        -> ("Зажим", "де4.835.001")
    # ===================================================================
    our_code_patterns = [
        # АМФИ/ГВАТ/ИГНД с точками: XXX.123456.789 (группы цифр могут быть разной длины)
    ...
    for rx in our_code_patterns:
        ...
        # нормализуем пробелы внутри "де 4" -> "де4"
    ...
    # Паттерны проверяются по порядку, поэтому более специфичные должны быть первыми
    tu_patterns = [
        # ТУ с пробелом и цифрами (ВЫСШИЙ ПРИОРИТЕТ)
        ...
        # ВАЖНО: НЕТ \s* между \d+ и [А-ЯЁ] чтобы не захватывать "1    НЩ0.364" (цифра из названия)
        (r'(\d+[А-ЯЁа-яё]+\d+\.\d+[\d\.\-\/А-ЯЁа-яёA-Za-z]*\s*ТУ[\d\.\-\/А-ЯЁа-яёA-Za-z]*)', re.IGNORECASE),
        ...
        # Этот fallback уже не нужен - дубликат исправленного паттерна выше
