...
class PDFExporter:
    ...
    def _get_sheet_data(self, sheet: Worksheet, max_rows: int = 1000) -> List[List]:
        ...
        cell_style = ParagraphStyle(
            ...
            parent=self.styles['Normal'],
            fontName=self.cyrillic_font,
            fontSize=7 if is_compact else 7,  # Увеличен для компактных таблиц (SUMMARY/SOURCES)
            leading=9 if is_compact else 10,  # межстрочный интервал
        ...
        cell_style_header = ParagraphStyle(
            ...
            parent=self.styles['Normal'],
            fontName=self.cyrillic_font_bold,
            fontSize=8.5 if is_compact else 9,  # Увеличен для компактных таблиц
            leading=10 if is_compact else 11,
            wordWrap='CJK'
        )
        ...
        for idx, row in enumerate(sheet.iter_rows(min_row=1, max_row=max_row, max_col=max_col)):
            ...
            for idx_cell, cell in enumerate(row):
                ...
                # Переименовываем заголовки колонок только в первой строке
                if idx == 0:
                    if value == '№ п/п':
                        value = '№'
    ...
    def _create_table(self, data: List[List], sheet: Worksheet, is_compact: bool = False, sheet_name: str = None) -> Table:
        ...
        if num_cols == 9:
            ...
            # ТРУ/РКМ формат (6 колонок)
            # Нужно определить, это ТРУ или РКМ
            # ТРУ: Артикул, Наименование, Количество, Цена, Стоимость, Ответственные
            # РКМ: №, Наименование, Цена, Стоимость, Документы, Поставщик
            
            is_rkm = False
            # Проверяем заголовки (в первой строке данных)
            if data and len(data) > 0:
                header_row = data[0]
                # В РКМ 3-я колонка (индекс 2) это Цена, в ТРУ - Количество
                # Но мы уже обернули в Paragraph, поэтому проверяем текст
                try:
                    h3_text = header_row[2].text.lower()
                    if 'цена' in h3_text or 'price' in h3_text:
                        is_rkm = True
                except:
                    pass
            ...
            if is_rkm:
                # РКМ формат
                # User request: 
                # Поставщик: уменьшить в 3 раза (было 75 -> 25)
                # Документы: увеличить (за счет остальных) (было 45 -> 120)
                # Наименование: уменьшить на 30% (было 90 -> 63 -> округлим до 60)
                # №: уменьшить на 50% (было 15 -> 7.5 -> округлим до 8)
                col_widths = [
                    8*mm,    # 1. №
                    60*mm,   # 2. Наименование
                    25*mm,   # 3. Цена
                    25*mm,   # 4. Стоимость (Затраты)
                    120*mm,  # 5. Документы (очень широкая)
                    25*mm    # 6. Поставщик (узкая, только название)
                ]
            else:
                # ТРУ формат (старый)
                col_widths = [
