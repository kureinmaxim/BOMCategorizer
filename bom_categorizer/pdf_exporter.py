...
class PDFExporter:
    ...
    def _create_table(self, data: List[List], sheet: Worksheet, is_compact: bool = False, sheet_name: str = None) -> Table:
        ...
        if sheet and hasattr(sheet, 'iter_rows'):
            # Заголовки обычно в 1-й строке
            for h_row in sheet.iter_rows(min_row=1, max_row=1, values_only=True):
                for val in h_row:
                    if val:
                        s_val = str(val).lower()
                        # РКМ специфичные слова
                        if any(kw in s_val for kw in ['поставщик', 'документы', 'затраты', 'обосновывающие документы']):
                            is_rkm = True
                            break
                if is_rkm: break
        ...
        if is_rkm:
            if num_cols == 7:
                ...
                # Сумма: 8+60+9+15+22+112+51 = 277mm
                col_widths = [
                    8*mm,    # 1. № (шире для 3 цифр)
                    60*mm,   # 2. Наименование
                    9*mm,    # 3. шт. (минимум для заголовка)
                    15*mm,   # 4. Цена (минимум для заголовка)
                    22*mm,   # 5. Стоимость (без переноса)
                    112*mm,  # 6. Документы
                    51*mm    # 7. Поставщик (+70%)
                ]
            else:
                # РКМ формат (6 колонок: №, Наименование, Цена, Стоимость, Документы, Поставщик)
                col_widths = [
                    10*mm,   # 1. №
                    70*mm,   # 2. Наименование
                    25*mm,   # 3. Цена
                    25*mm,   # 4. Стоимость (без переноса)
                    93*mm,   # 5. Документы
                    54*mm    # 6. Поставщик (+70%)
                ]
        elif num_cols == 9:
            # 9 колонок — оптимизировано для A4 landscape (277mm доступно)
            # Все заголовки должны помещаться в одну строку
