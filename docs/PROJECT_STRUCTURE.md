...
## 🗂️ Организация файлов (v3.3.0 / v4.4.1)

> 🎨 **НОВОЕ:** Проект поддерживает две независимые версии GUI:
> - **Standard v3.3.0** (Tkinter) - стабильная
> - **Modern Edition v4.4.1** (PySide6) - современная с AI-подсказками, Drag & Drop, CLI и кроссплатформенной конвертацией .doc
> - **🍎 macOS Retina Support** - исправлены мелкие шрифты на Retina дисплеях
> - **🔄 LibreOffice Integration** - автоматическая конвертация .doc → .docx на macOS/Linux
> - **📁 Config Path Fix** - правильные пути для установленных версий (v4.4.1+)
...
│   ├── gui_qt.py                         # ✨ PySide6 GUI Modern Edition (~3600 строк)
│   ├── dialogs_qt.py                     # ✨ PySide6 диалоги (PIN, статистика, классификация)
│   ├── gui_sections_qt.py                # ✨ PySide6 секции интерфейса (рефакторинг)
│   ├── gui_menu_qt.py                    # ✨ PySide6 меню и горячие клавиши
...
│   ├── workers_qt.py                     # ✨ PySide6 фоновые процессы (QThread)
│   ├── drag_drop_qt.py                   # 🎯 Улучшенный Drag & Drop (~430 строк)
│   ├── cli_interactive.py                # 💻 Интерактивная командная строка (~550 строк)
│   ├── pdf_exporter.py                   # 📄 Экспорт результатов в PDF (~380 строк)
│   ├── ai_classifier_qt.py               # 🤖 AI-подсказки классификации (~450 строк)
│   │
│   ├── component_database.py            # База данных компонентов (постоянное хранение)
│   ├── config_manager.py                # 🆕 Создание config файлов из шаблонов (v4.2.3+)
│   └── podborka_extractor.py            # Извлечение компонентов для подборки
│
├── ⚙️ config.json.template               # 🆕 Шаблон конфигурации Standard (в Git)
├── ⚙️ config_qt.json.template            # 🆕 Шаблон конфигурации Modern Edition (в Git)
├── ⚙️ config.json                        # ✅ Локальная конфигурация Standard (не в Git)
├── ⚙️ config_qt.json                     # ✨ Локальная конфигурация Modern Edition (не в Git)
...
│
├── 🔧 update_version.py                  # 🆕 Централизованное управление версиями (v4.2.3+)
├── 🔧 sync_installer_versions.py         # 🆕 Синхронизация версий в .iss файлах (v4.2.3+)
...
│   ├── reportlab-*.whl                   # 🆕 Для экспорта в PDF
│   └── ... (ещё 9+ пакетов)
│
├── 📄 DRAG_DROP_README.md                # Документация улучшенного Drag & Drop
├── 📄 AI_CLASSIFIER_README.md            # Документация AI-подсказок для классификации
├── 📄 EXPERIMENTAL_FEATURES.md           # Описание расширенных функций
├── 📄 MACOS_RETINA_FIX.md                # 🍎 Исправление шрифтов на macOS Retina
│
├── 📚 docs/                              # Документация
...
│   ├── 🔧 BAT_FILES_GUIDE.md             # 🆕 Расширенное описание BAT файлов (v4.3.6+)
...
│   │
│   ├── 🗄️ DATABASE_GUIDE.md              # Полное руководство по базе данных (v3.0.0+, объединенное)
│   ├── 🔧 VERSION_MANAGEMENT.md          # 🆕 Управление версиями проекта (v4.2.3+)
│   ├── 🖥️ PLATFORM_COMPARISON.md         # 🆕 Сравнение Windows vs macOS (v4.2.3+)
│   └── 🍎 MACOS_RETINA_FIX.md            # 🆕 Исправление шрифтов на macOS Retina (v4.3.6+)
│
...
| Файл | Описание | Приоритет |
|------|----------|-----------|
| `docs/DATABASE_GUIDE.md` | 📖 Полное руководство по базе данных (объединенное) | ⭐⭐⭐ |

### 🔵 Для разработчиков
| Файл | Описание | Приоритет |
|------|----------|-----------|
| `docs/VERSION_MANAGEMENT.md` | 🆕 Управление версиями (v4.2.3+) | ⭐⭐⭐ |
| `docs/PLATFORM_COMPARISON.md` | 🆕 Сравнение Windows vs macOS (v4.2.3+) | ⭐⭐⭐ |
...
## 🆕 Централизованное управление версиями (v4.2.3+)

### 📦 Система шаблонов конфигураций

Начиная с версии 4.2.3, проект использует централизованную систему управления версиями:

**Единый источник правды:**
```
config.json.template       → Standard Edition (в Git)
config_qt.json.template    → Modern Edition (в Git)
         ↓
config.json               → Локальная конфигурация (НЕ в Git)
config_qt.json            → Локальная конфигурация (НЕ в Git)
```

**Преимущества:**
- ✅ Версия хранится ТОЛЬКО в шаблонах
- ✅ Локальные настройки (scale_factor, window size) не попадают в Git
- ✅ Автоматическая синхронизация всех скриптов сборки
- ✅ Одна команда для обновления версии везде

### 🔧 Утилиты управления версиями

**1. update_version.py** - главная утилита:
```bash
# Показать текущие версии
python update_version.py status

# Обновить версию Modern Edition
python update_version.py set modern 4.4.1

# Синхронизировать файлы сборки
python update_version.py sync
```

**2. sync_installer_versions.py** - синхронизация .iss файлов:
- Автоматически обновляет версии в `installer_clean.iss` и `installer_qt.iss`
- Вызывается автоматически из `update_version.py`

**3. config_manager.py** - создание config из шаблонов:
- При первом запуске приложения создает `config.json` из `config.json.template`
- Интегрирован в `gui.py` и `gui_qt.py`

### 📖 Полная документация

Подробности в **[docs/VERSION_MANAGEMENT.md](VERSION_MANAGEMENT.md)** (484 строки)
...
|--------|------|-------------------|
| **3.3.0/4.4.1** | **16.11.2025** | **🎉 ТЕКУЩАЯ ВЕРСИЯ** - 📁 Config Path Fix для установленных версий! Правильные пути к config_qt.json, исправлено сохранение настроек PDF поиска, правильное открытие папки установки в футере. 🍎 Исправлены шрифты на macOS Retina! 🔄 Автоматическая конвертация .doc через LibreOffice (macOS/Linux)! AI-подсказки для классификации, интеграция с Claude/GPT/Ollama, модульный рефакторинг (11 модулей, ~5500 строк), глобальный поиск, поиск PDF, Drag & Drop, CLI и PDF экспорт |
| 3.3.0/4.3.6 | 15.11.2025 | 🍎 Исправлены мелкие шрифты на macOS Retina дисплеях! High DPI поддержка, правильное масштабирование, глобальный шрифт 13pt для macOS. Автоматическая синхронизация версий через update_version.py |
| 3.3.0/4.3.5 | 15.11.2025 | Исправлена проблема с отображением старой версии на macOS. Улучшена загрузка config_qt.json из .app bundle |
| 3.3.0/4.3.0 | 12.11.2025 | AI-подсказки для классификации! Интеграция с Claude/GPT/Ollama, модульный рефакторинг (11 модулей, ~5500 строк), глобальный поиск, поиск PDF, Drag & Drop, CLI и PDF экспорт |
| 3.3.0/4.2.3 | 12.11.2025 | 🆕 Централизованная система управления версиями! update_version.py, config шаблоны, автоматическая синхронизация .iss файлов. Защита локальных настроек (config в .gitignore). Модульный рефакторинг (10+ модулей) |
| 3.3.0/4.2.2 | 12.11.2025 | Модульный рефакторинг Modern Edition! 10 модулей (~5000 строк), глобальный поиск, поиск PDF, Drag & Drop, CLI и PDF экспорт. Горячие клавиши портированы в Standard v3.3.0 |
| 3.3.0/4.1.0 | 11.11.2025 | Enhanced UX! Горячие клавиши (Ctrl+O/R/Q), Drag & Drop, полная система помощи (F1, база знаний), GitHub интеграция, исправлено масштабирование, режимы работы |
| 3.2.2/4.0.0 | 08.11.2025 | Две версии GUI! Standard v3.2.2 (Tkinter) и Modern Edition v4.0.0 (PySide6, современный дизайн). Диалоги выбора в build скриптах, отдельные конфиги. EDITIONS.md |
...
**Разработчикам:**
1. 🔧 `docs/VERSION_MANAGEMENT.md` - 🆕 управление версиями проекта (v4.2.3+)
2. 🖥️ `docs/PLATFORM_COMPARISON.md` - 🆕 сравнение Windows vs macOS (v4.2.3+)
3. 📊 `ANALYSIS_PROJECT.md` - детальный анализ кодовой базы, архитектура, паттерны
4. 🔧 `BUILD.md` - как собрать инсталлятор (автоматически через `build_installer.py`)
5. 📦 `docs/OFFLINE_INSTALLER.md` - про офлайн режим
6. 📁 `docs/PROJECT_STRUCTURE.md` - структура проекта (вы здесь)
7. 🐍 `update_version.py` - централизованное управление версиями
7. 🐍 `build_installer.py` - скрипт автоматической сборки
...
## 📌 Основные возможности v3.3.0 / v4.2.2

### 🎨 Две версии GUI (v3.3.0/v4.2.2):
...
- **Конфиг:** edition "Standard", version "3.3.0"
- **Обновления v3.3.0:**
  - Горячие клавиши (портированы из Modern Edition)
  - Улучшенная система помощи
  - Оптимизация производительности

#### Modern Edition v4.4.1 (PySide6) ✨
- **Современная версия** - современный дизайн с AI
- **PySide6 GUI** - Qt фреймворк, лучшая кастомизация
- **Приглушенная цветовая палитра** - #5B9BD5, #67B279, #95A5A6
- **Увеличенные шрифты** - 13-14pt для лучшей читаемости (13pt на macOS Retina)
- **QSS стили** - аналог CSS для оформления
- **Размер:** ~35-50 MB инсталлятор (включая PySide6, AI optional)
- **Файлы:** app_qt.py, config_qt.json, gui_qt.py, dialogs_qt.py, ai_classifier_qt.py, installer_qt.iss
- **Конфиг:** edition "Modern Edition", секция "ai_classifier"
...
- 🤖 **AI-подсказки** - автоматическая классификация через Claude/GPT/Ollama (экспертный режим)
- 🍎 **macOS Retina Support** - правильное отображение на Retina дисплеях (High DPI)
- 🔄 **Кроссплатформенная конвертация .doc** - Windows (MS Word), macOS/Linux (LibreOffice)
- 📁 **Config Path Fix** - правильные пути к config_qt.json для установленных версий (v4.4.1+)
...
*Создано: 06.10.2025*  
*Обновлено: 16.11.2025 (Config Path Fix v4.4.1, macOS Retina support + LibreOffice integration)*  
*Версия проекта: Standard 3.3.0 / Modern Edition 4.4.1*  
*Новое: 📁 Config Path Fix для установленных версий, 🍎 Исправлены шрифты на macOS Retina, 🔄 Автоматическая конвертация .doc через LibreOffice*  
*Статус: ✅ Актуально*
