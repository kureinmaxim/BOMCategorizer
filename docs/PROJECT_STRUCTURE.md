...
├── 📄 ANALYSIS_PROJECT.md                # Детальный анализ кодовой базы (v3.1.3)
...
│
├── 📦 bom_categorizer/                   # Модульная структура (v3.1.3)
│   ├── __init__.py                       # Инициализация модуля (метаданные v3.1.3)
...
│   ├── gui.py                            # Tkinter GUI (PIN защита, интерактивная классификация, управление БД)
...
│
├── ⚙️ config.json                        # Конфигурация приложения (PIN: 1234, настройки)
...
├── 🔧 database_stats.bat                 # 🆕 Статистика БД (двойной клик) (v3.0.0)
├── 🔧 database_backup.bat                # 🆕 Резервная копия БД (v3.0.0)
├── 🔧 database_export.bat                # 🆕 Экспорт БД в Excel (v3.0.0)
├── 🔧 manage_database.bat                # 🆕 Главное меню управления БД (v3.0.0)
├── 🔧 merge_component_database.py        # Слияние баз при обновлении (v2.0.33+)
...
├── 🔧 repair_install.ps1                 # Скрипт восстановления (v2.0.33+)
├── 🔧 repair_install.bat                 # Обертка для восстановления (v2.0.33+)
├── 🔧 installer_clean.iss                # Скрипт Inno Setup
├── 📄 ЕСЛИ_НЕ_ЗАПУСКАЕТСЯ.txt             # Инструкция для пользователя (v2.0.33+)
│
...
|--------|------|-------------------|
| **3.1.2** | **05.11.2025** | **🎉 ТЕКУЩАЯ ВЕРСИЯ** - Управление БД через GUI! Кнопка импорта всех компонентов из выходного файла, статистика, экспорт/импорт, резервное копирование прямо в интерфейсе. Умная генерация имен файлов, корректное извлечение ТУ с суффиксами, нормализация допусков резисторов |
