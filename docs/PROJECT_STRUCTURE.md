...
│
├── 📦 bom_categorizer/                   # Модульная структура (v1.8.2)
│   ├── __init__.py                       # Инициализация модуля
│   ├── main.py                           # CLI точка входа
│   ├── classifiers.py                    # Классификация компонентов (улучшенная v1.8.2)
│   ├── parsers.py                        # Парсеры TXT/DOCX/Excel
│   ├── formatters.py                     # Форматирование данных (исправлено извлечение ТУ, сортировка)
│   ├── excel_writer.py                   # Запись Excel с форматированием
│   ├── txt_writer.py                     # Генерация TXT отчетов
│   ├── utils.py                          # Утилиты и regex
│   └── gui.py                            # Tkinter GUI (добавлен перенос компонентов v1.8.2)
│
...
|--------|------|-------------------|
| 1.8.3 | 14.10.2025 | **Унификация DOCX/XLSX**, извлечение ТУ перед агрегацией, разделение всех категорий по источникам |
...
## 📌 Основные возможности v1.8.2

### 🆕 Новые функции:
- 🔄 **Перенос компонентов в GUI** - ручной перенос неправильно классифицированных компонентов в "Не распределено" для переклассификации
- 🔍 **Умный поиск** - поиск по частичному совпадению названия (регистронезависимый)
- 📊 **Интеграция с интерактивной классификацией** - после переноса можно сразу запустить переклассификацию
- 📂 **Разделение по источникам** - все категории теперь разделяются пустыми строками между разными исходными файлами

### ✅ Исправления v1.8.2:
- **Унификация DOCX/XLSX** - DOCX файлы обрабатываются по тому же алгоритму что и XLSX (извлечение ТУ ПЕРЕД агрегацией)
- **Корректная агрегация** - одинаковые компоненты из разных форматов правильно объединяются
- **Разделение всех категорий** - пустые строки между источниками добавляются во всех категориях (не только отладочные платы)
- **Сортировка по номиналу** - исправлена для резисторов, конденсаторов, индуктивностей (теперь 27 Ом → 220 Ом → 536 Ом → 845 Ом → 1 кОм → 12 кОм → 82.5 кОм → 681 кОм)
- **Извлечение ТУ** - корректная обработка кодов с дефисами (АЕЯР431200424-07ТУ, АЕЯР431320515-01ТУ)
- **Извлечение производителя** - импортные компоненты с "ф. Производитель" корректно обрабатываются
- **Классификация оптики** - компоненты с "photonic", "optical" больше не попадают в "Микросхемы"
- **СВЧ компоненты** - аттенюаторы, делители, ответвители от Qualwave, Mini-Circuits, A-INFO, Quantic PMI в "Отладочные платы"
- **Модули связи** - NT1, Ebyte корректно определяются как "Отладочные платы"
