...
2. [Команды AI настроек](#-команды-ai-настроек-новое)
3. [Обработка BOM файлов](#-обработка-bom-файлов-split_bompy)
4. [AI поиск компонентов](#-ai-поиск-компонентов-ai_searchpy)
5. [Управление версиями](#-управление-версиями-update_versionpy)
6. [Синхронизация API](#-синхронизация-api-ключа-telegramhelper)
...
#### 🔹 Команды AI настроек (НОВОЕ!)

| Команда | Алиас | Описание |
|---------|-------|----------|
| `aiinfo` | `ai` | Показать все настройки AI |
| `aiprovider` | `provider` | Показать/сменить провайдера |
| `aimodel` | `model` | Показать/сменить модель AI |
| `aimodels` | `models` | Список всех доступных моделей |
...
✅ AI провайдер изменён на ANTHROPIC

>>> aimodel claude-opus-4-5-20251101
✅ Модель Anthropic изменена на claude-opus-4-5-20251101
...
Встроенные команды CLI для управления AI провайдерами и моделями без выхода из приложения.

### aiinfo (ai) — Показать все настройки
...
### aiprovider — Смена провайдера AI

```
# Показать текущий провайдер
>>> aiprovider
📍 Текущий AI провайдер: TELEGRAM
...
| Провайдер | Описание |
|-----------|----------|
| `telegram` | Telegram Bot API (рекомендуется, не нужны собственные ключи) |
| `anthropic` | Anthropic Claude напрямую |
| `openai` | OpenAI GPT напрямую |

### aimodel — Смена модели AI
...
📍 Активный провайдер: ANTHROPIC

🔹 Anthropic: claude-3-5-sonnet-20241022
🔹 OpenAI: gpt-4
🔹 Telegram: (модель настраивается на сервере через /ch_model)
...
🔹 Telegram Bot:
   • Модель настраивается на сервере
   • Используйте команду /ch_model в боте
```

### Примеры рабочего процесса

```bash
# 1. Проверить текущие настройки AI
>>> ai

# 2. Посмотреть доступные модели
>>> aimodels

# 3. Сменить на более мощную модель Anthropic
>>> aimodel claude-opus-4-5-20251101

# 4. Или переключиться на OpenAI
>>> aiprovider openai
>>> aimodel gpt-4-turbo
...
> 💡 **Совет:** Для Telegram Bot модель настраивается на сервере через команду `/ch_model` в боте.
...
| **`aiinfo` / `ai`** | **Показать все настройки AI** |
| **`aiprovider`** | **Сменить провайдера AI** |
| **`aimodel`** | **Сменить модель AI** |
| **`aimodels`** | **Список всех моделей** |
