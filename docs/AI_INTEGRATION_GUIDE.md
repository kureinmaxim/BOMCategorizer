...
2. [Протокол взаимодействия](#-протокол-взаимодействия)
3. [Защита от неправомерного использования](#-защита-от-неправомерного-использования)
4. [Используемые технологии](#-используемые-технологии)
5. [Компоненты системы](#-компоненты-системы)
6. [API провайдеры](#-api-провайдеры)
7. [Система промптов](#-система-промптов)
8. [Настройка и конфигурация](#-настройка-и-конфигурация)
9. [Безопасность API ключей](#-безопасность-api-ключей)
...
2. **TelegramHelper** — веб-сервис (FastAPI) с AI-функциями
3. **Три режима работы:**
   - Прямой вызов Anthropic Claude API
   - Прямой вызов OpenAI GPT API
   - Через TelegramHelper API (проксирование)
...
```json
{
  "api_keys": {
    "anthropic": "sk-ant-...",
    "openai": "sk-proj-...",
    "telegram_url": "https://api.example.com/ai_query",
    "telegram_key": "your_api_key",
    "telegram_hmac_secret": "your_hmac_secret"
  },
  "security": {
...
2. Включите **Экспертный режим** (двойной клик на имя разработчика)
3. Откройте **Поиск PDF → Настройки**
4. Перейдите на вкладку **API Ключи**
5. Введите ваши ключи
6. Нажмите **Сохранить**
