# Анализ кодовой базы: BOM Categorizer v3.3.0 / v4.4.5
# Codebase Analysis: BOM Categorizer Standard & Modern Edition

> **Дата анализа / Analysis Date:** 21.11.2025  
> **Тип проекта / Project Type:** Desktop приложение (Python + Tkinter/PySide6) / Desktop Application  
> **Версии / Versions:** Standard v3.3.0 (Tkinter) + Modern Edition v4.4.5 (PySide6)  
...
> **📁 Config Management:** Правильные пути для установленных версий (v4.4.1+)
> **📐 Window Sizes:** Индивидуальные размеры окна для каждого режима (v4.4.3+)
> **📝 TXT Export:** Полная поддержка ТУ и импортных компонентов (v4.4.5+)
> **🔧 Podbor Filter:** Корректная фильтрация подборов без потери основных компонентов (v4.4.5+)
...
├── ⚙️ config_qt.json                # ✨ Конфигурация Modern Edition v4.4.3
...
└── 📝 Прочее:
    ...
    ├── CREATE_GIT_RELEASE.md        # Руководство по созданию GitHub релизов
    ├── DRAG_DROP_README.md          # Документация улучшенного D&D
    ├── EXPERIMENTAL_FEATURES.md     # Описание расширенных функций
...
class BOMCategorizerMainWindow(QMainWindow):
    def __init__(self):
        # Загружаем config_qt.json (v4.4.3)
        self.cfg = load_config()  # из config_qt.json (использует get_config_path())
...
5. **Управление конфигурацией (v4.4.1+):**
   - Функция `get_config_path()` для единой логики определения пути к `config_qt.json`
   - Поддержка режима разработки и установленных версий
   - Автоматическое создание папок при необходимости
   - Правильное сохранение всех настроек (UI, PDF поиск, масштаб, тема)

6. **Размеры окна по режимам (v4.4.3+):**
   - Индивидуальные размеры окна для каждого режима (simple: 730×560, advanced: 730×790, expert: 730×1160)
   - Автоматическая смена размера при переключении режимов
   - Независимое сохранение размеров для каждого режима
   - Миграция старого формата конфигурации
...
## 🆕 Что нового в v3.3.0 / v4.4.5

### v4.4.5 - Modern Edition (21.11.2025) ⭐ ТЕКУЩАЯ
- 🔧 **КРИТИЧНО: Исправлен порядок операций** - фильтр подборов теперь выполняется ДО агрегации
  - Решена проблема удаления основных компонентов при включенном "Исключить подборы"
  - Подборы корректно фильтруются без объединения с основными элементами
- 🐛 **Исправлен regex фильтра подборов** - паттерн упрощен с `r'\(п/б\s|\\(зам\s|\(подбор\s'` на `r'\(п/б|\(зам'`
- 📋 **Расширена поддержка ТУ-кодов** - добавлена поддержка дефисов в номере (БКЮС.670109.002-01ТУ)
- 📄 **TXT файлы с колонкой ТУ** - применяется `format_excel_output` перед записью TXT
  - Единый источник данных для Excel и TXT
  - Колонка ТУ/производителя теперь в TXT файлах
- 🌍 **Улучшено определение импортных компонентов** - добавлены паттерны ГОСТ
  - Резисторы: Р\d+, С\d+, НР\d+, МЛТ, СП\d+
  - Конденсаторы: К\d+, КМ, КД
  - Полупроводники: \d[ДСТ]\d+, КД\d+, КТ\d+
  - Микросхемы: \d{3,4}[А-ЯЁ]{2}\d (1272ПН3Т, 140УД17А)
- ♻️ **Удаление дублирования единиц** - `remove_duplicate_suffix()` в `excel_writer.py`
  - Удаляет "кОм ± 1% - М кОм ± 1% - М" → "кОм ± 1% - М"
  - Работает для разных единиц: "МОм ± 1% - M кОм ± 1% - M" → "МОм ± 1% - M"
- 🎨 **GUI improvements** - чекбокс в одной строке, улучшенные отступы (8-10px)

### v4.4.3 - Modern Edition (20.11.2025)
- 📐 **Размеры окна для каждого режима** - индивидуальные размеры для simple/advanced/expert
  - Simple режим: 730×560 (компактный)
  - Advanced режим: 730×790 (расширенный)
  - Expert режим: 730×1160 (максимальный)
  - Автоматическая смена размера при переключении режимов
  - Независимое сохранение размеров для каждого режима в `config_qt.json`
  - Миграция старого формата конфигурации (обратная совместимость)
  - Методы `_apply_window_size_for_mode()` и обновленный `save_window_size_to_config()`
- 📖 **Документация:** Обновлен `CHANGELOG.md` с полным описанием изменений

### v4.4.1 - Modern Edition (16.11.2025)
- 📁 **Config Path Fix для установленных версий** - критическое исправление
  - Функция `get_config_path()` для единой логики определения пути к `config_qt.json`
  ...
  - Обновлены все функции сохранения настроек (`save_ui_preferences`, `save_pdf_search_config`, `save_search_dirs_to_config`)
  - Правильное открытие папки установки в футере (вместо папки базы данных)
  - Нормализация путей при сохранении пользовательских папок PDF поиска
- 🔧 **Исправления:**
  - Сохранение настроек PDF поиска в правильном месте
  - Обновление конфига в родительском окне после сохранения
  - Создание папки установки при необходимости
- 📖 **Документация:** Обновлен `CHANGELOG.md` с полным описанием изменений

### v4.3.7 - Modern Edition (15.11.2025)
...
*Документ обновлен: 21.11.2025*  
*Версия проекта: Standard 3.3.0 / Modern Edition 4.4.5*  
*Новое: 🔧 Критичные исправления (v4.4.5), 📄 TXT с ТУ (v4.4.5), ♻️ Удаление дублирования (v4.4.5), 📐 Window Sizes per Mode (v4.4.3), 🍎 macOS Retina support (v4.3.6)*  
