...
def main():
    ...
    parser.add_argument("--out", default="out", help="Output directory for CSVs (optional)")
    ...
    if args.assign_json and os.path.exists(args.assign_json):
        ...
        if isinstance(rules, list):
            ...
            for i, rule in enumerate(rules, start=1):
                ...
                if contains:
                    blob = (
                        df.get("description", "").astype(str).str.lower().fillna("") + " " +
                        df.get("value", "").astype(str).str.lower().fillna("") + " " +
                        df.get("part", "").astype(str).str.lower().fillna("") + " " +
                        df.get("reference", "").astype(str).str.lower().fillna("")
                    )
